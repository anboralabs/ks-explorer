 ...
 */
public class DViewCrl extends CertEditor {
   ...
   */
  public DViewCrl(Project project, VirtualFile file, X509CRL crl) {
    super(project, file);
    this.crl = crl;
    initComponents();
  }
  ...
  private void initComponents() {
    ...
    jdnIssuer = new JDistinguishedName(res.getString("DViewCrl.Issuer.Title"),
                                       30, false);
    ...
    jlEffectiveDate =
        new JLabel(res.getString("DViewCrl.jlEffectiveDate.text"));
    ...
    jtfEffectiveDate.setToolTipText(
        res.getString("DViewCrl.jtfEffectiveDate.tooltip"));
    GridBagConstraints gbc_jtfEffectiveDate =
        (GridBagConstraints)gbcCtrl.clone();
    ...
    jtfNextUpdate.setToolTipText(
        res.getString("DViewCrl.jtfNextUpdate.tooltip"));
    GridBagConstraints gbc_jtfNextUpdate = (GridBagConstraints)gbcCtrl.clone();
    gbc_jtfNextUpdate.gridy = 3;

    jlSignatureAlgorithm =
        new JLabel(res.getString("DViewCrl.jlSignatureAlgorithm.text"));
    ...
    jtfSignatureAlgorithm.setToolTipText(
        res.getString("DViewCrl.jtfSignatureAlgorithm.tooltip"));
    GridBagConstraints gbc_jtfSignatureAlgorithm =
        (GridBagConstraints)gbcCtrl.clone();
    gbc_jtfSignatureAlgorithm.gridy = 4;

    /*jbCrlExtensions = new
    JButton(res.getString("DViewCrl.jbCrlExtensions.text"));

    PlatformUtil.setMnemonic(jbCrlExtensions,
    res.getString("DViewCrl.jbCrlExtensions.mnemonic").charAt(0));
    ...
    PlatformUtil.setMnemonic(jbCrlAsn1,
    res.getString("DViewCrl.jbCrlAsn1.mnemonic").charAt(0));
    ...
    for (int i = 0; i < jtRevokedCerts.getColumnCount(); i++) {
      ...
      column.setHeaderRenderer(new RevokedCertsTableHeadRend(
          jtRevokedCerts.getTableHeader().getDefaultRenderer()));
    ...
    jspRevokedCertsTable = PlatformUtil.createScrollPane(
        jtRevokedCerts, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
        ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
    jspRevokedCertsTable.getViewport().setBackground(
        jtRevokedCerts.getBackground());
    ...
    jpRevokedCertsTable.setBorder(
        new CompoundBorder(new EtchedBorder(), new EmptyBorder(5, 5, 5, 5)));

    /*jbCrlEntryExtensions = new
    JButton(res.getString("DViewCrl.jbCrlEntryExtensions.text"));
  ...
  private void populateDialog() {
    ...
    jdnIssuer.setDistinguishedName(
        X500NameUtils.x500PrincipalToX500Name(crl.getIssuerX500Principal()));

    jtfEffectiveDate.setText(StringUtils.formatDate(effectiveDate));

    if (effective) {
      jtfEffectiveDate.setText(MessageFormat.format(
          res.getString("DViewCrl.jtfEffectiveDate.noteffective.text"),
    ...
    if (updateAvailable) {
      jtfNextUpdate.setText(MessageFormat.format(
          res.getString("DViewCrl.jtfNextUpdate.updateavailable.text"),
    ...
    SignatureType sigAlg =
        SignatureType.resolveOid(crl.getSigAlgOID(), crl.getSigAlgParams());
    String sigAlgName =
        (sigAlg != null) ? sigAlg.friendly() : crl.getSigAlgName();
    ...
    X509CRLEntry[] revokedCerts =
        revokedCertsSet.toArray(new X509CRLEntry[revokedCertsSet.size()]);
    RevokedCertsTableModel revokedCertsTableModel =
        (RevokedCertsTableModel)jtRevokedCerts.getModel();
  ...
  res.getString("DViewCrl.Extensions.Title"), crl);
  ...
  private void displayCrlEntryExtensions() {
      ...
      if (row != -1) {
          BigInteger serialNumber = (BigInteger) jtRevokedCerts.getValueAt(row,
  0);
