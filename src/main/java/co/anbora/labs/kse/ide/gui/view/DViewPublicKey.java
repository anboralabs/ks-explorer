 ...
 */
public class DViewPublicKey extends CertEditor {
   ...
   * @throws CryptoException A problem was encountered getting the public key's
   *     details
   */
  public DViewPublicKey(Project project, VirtualFile file, PublicKey publicKey)
  ...
  private PublicKey convertKey(PublicKey publicKey) throws CryptoException {
    // convert public key object from whatever class it currently is (depends on
    // Java version) to a BC object
  ...
  private void initComponents() throws CryptoException {
    ...
    jtfAlgorithm.setToolTipText(
        res.getString("DViewPublicKey.jtfAlgorithm.tooltip"));
    ...
    jtfKeySize.setToolTipText(
        res.getString("DViewPublicKey.jtfKeySize.tooltip"));
    ...
    jtaEncoded.setToolTipText(
        res.getString("DViewPublicKey.jtfEncoded.tooltip"));
    ...
    jcfFingerprint = new JPublicKeyFingerprint(projectArg, 25);

    jbPem = new JButton(res.getString("DViewPublicKey.jbPem.text"));
    PlatformUtil.setMnemonic(
        jbPem, res.getString("DViewPublicKey.jbPem.mnemonic").charAt(0));
    ...
    PlatformUtil.setMnemonic(
        jbFields, res.getString("DViewPublicKey.jbFields.mnemonic").charAt(0));
    ...
    PlatformUtil.setMnemonic(
        jbAsn1, res.getString("DViewPublicKey.jbAsn1.mnemonic").charAt(0));
  ...
  private void populateDialog() throws CryptoException {
    ...
    if (keyLength != null) {
      jtfKeySize.setText(MessageFormat.format(
          res.getString("DViewPublicKey.jtfKeySize.text"), "" + keyLength));
    } else {
      jtfKeySize.setText(MessageFormat.format(
          res.getString("DViewPublicKey.jtfKeySize.text"), "?"));
    }

    jtfFormat.setText(publicKey.getFormat());

    jtaEncoded.setText(
        new BigInteger(1, publicKey.getEncoded()).toString(16).toUpperCase());
    jtaEncoded.setCaretPosition(0);

    jcfFingerprint.setPublicKey(publicKey);

    jbFields.setEnabled((publicKey instanceof RSAPublicKey) ||
                        (publicKey instanceof DSAPublicKey) ||
                        (publicKey instanceof ECPublicKey) ||
                        (publicKey instanceof BCEdDSAPublicKey));
  }

  /*private void pemEncodingPressed() {
      try {
          DViewPem dViewCsrPem = new DViewPem(this,
  res.getString("DViewPublicKey.Pem.Title"), publicKey);
  ...
  DViewAsymmetricKeyFields(this, publicKey);
