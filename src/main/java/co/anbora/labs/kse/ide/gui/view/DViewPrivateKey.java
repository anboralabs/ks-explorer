...
public class DViewPrivateKey extends CertEditor {
  ...
  private static ResourceBundle resActions =
      ResourceBundle.getBundle("org/kse/gui/actions/resources");
   ...
   * @throws CryptoException A problem was encountered getting the private key's
   *     details
   */
  public DViewPrivateKey(Project project, VirtualFile file,
  ...
  public void loadPrivateKey(@NotNull PrivateKey privateKey)
      throws CryptoException {
    this.privateKey = privateKey;
    this.populateDialog();
  }
  ...
  private void initComponents() throws CryptoException {
    ...
    jtfAlgorithm.setToolTipText(
        res.getString("DViewPrivateKey.jtfAlgorithm.tooltip"));

    jlKeySize = new JLabel(res.getString("DViewPrivateKey.jlKeySize.text"));

    jtfKeySize = new JTextField();
    jtfKeySize.setEditable(false);
    jtfKeySize.setToolTipText(
        res.getString("DViewPrivateKey.jtfKeySize.tooltip"));

    jlFormat = new JLabel(res.getString("DViewPrivateKey.jlFormat.text"));

    jtfFormat = new JTextField();
    jtfFormat.setEditable(false);
    jtfFormat.setToolTipText(
        res.getString("DViewPrivateKey.jtfFormat.tooltip"));

    jlEncoded = new JLabel(res.getString("DViewPrivateKey.jlEncoded.text"));

    jtaEncoded = new JTextArea();
    jtaEncoded.setFont(
        new Font(Font.MONOSPACED, Font.PLAIN, LnfUtil.getDefaultFontSize()));
    jtaEncoded.setBackground(jtfFormat.getBackground());
    jtaEncoded.setEditable(false);
    jtaEncoded.setLineWrap(true);
    jtaEncoded.setToolTipText(
        res.getString("DViewPrivateKey.jtfEncoded.tooltip"));

    jspEncoded = PlatformUtil.createScrollPane(
        jtaEncoded, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
    ...
    PlatformUtil.setMnemonic(
        jbExport, res.getString("DViewPrivateKey.jbExport.mnemonic").charAt(0));
    ...
    PlatformUtil.setMnemonic(
        jbPem, res.getString("DViewPrivateKey.jbPem.mnemonic").charAt(0));
    ...
    PlatformUtil.setMnemonic(
        jbFields, res.getString("DViewPrivateKey.jbFields.mnemonic").charAt(0));
    ...
    PlatformUtil.setMnemonic(
        jbAsn1, res.getString("DViewPrivateKey.jbAsn1.mnemonic").charAt(0));
    ...
    pane.setLayout(
        new MigLayout("insets dialog", "[right]unrel[]", "[]unrel[]"));
    ...
    pane.add(jlEncoded, "");
    // pane.add(jspEncoded, "width 300lp:300lp:300lp, height 100lp:100lp:100lp,
    // wrap");
  ...
  /*private void exportPressed() {
      ...
      DExportPrivateKeyType dExportPrivateKeyType = new
     DExportPrivateKeyType((JFrame) this.getParent(), keyPairType);
      ...
      try
      {
          if (dExportPrivateKeyType.exportPkcs8()) {
              PrivateKeyUtils.exportAsPkcs8(privateKey, alias,(JFrame)
     this.getParent(), applicationSettings, resActions); } else if
     (dExportPrivateKeyType.exportPvk()) {
              PrivateKeyUtils.exportAsPvk(privateKey, alias, (JFrame)
     this.getParent(), applicationSettings, resActions); } else {
              PrivateKeyUtils.exportAsOpenSsl(privateKey, alias, (JFrame)
     this.getParent(), applicationSettings, resActions);
          }
      }
      catch (Exception ex) {
  ...
  private void populateDialog() throws CryptoException {
    if (privateKey != null) {
      ...
      if (privateKey instanceof ECPrivateKey) {
        jtfAlgorithm.setText(jtfAlgorithm.getText() + " (" +
      ...
      if (keyLength != null) {
        jtfKeySize.setText(MessageFormat.format(
            res.getString("DViewPrivateKey.jtfKeySize.text"), "" + keyLength));
      } else {
        jtfKeySize.setText(MessageFormat.format(
            res.getString("DViewPrivateKey.jtfKeySize.text"), "?"));
      }

      jtfFormat.setText(privateKey.getFormat());

      jtaEncoded.setText(new BigInteger(1, privateKey.getEncoded())
                             .toString(16)
                             .toUpperCase());
      jtaEncoded.setCaretPosition(0);

      jbFields.setEnabled((privateKey instanceof RSAPrivateKey) ||
                          (privateKey instanceof DSAPrivateKey) ||
                          (privateKey instanceof ECPrivateKey) ||
                          (privateKey instanceof BCEdDSAPrivateKey));
    }
  }
  ...
  res.getString("DViewPrivateKey.Pem.Title"), privateKey);
  ...
  private void fieldsPressed() {
      DViewAsymmetricKeyFields dViewAsymmetricKeyFields = new
  DViewAsymmetricKeyFields(this, privateKey);
