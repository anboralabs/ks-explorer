...
/**
 * Static helper methods for ECC stuff, mainly detection of available ECC
 * algorithms.
 */
public class EccUtil {
  ...
  static {
    ...
    if (sunECProvider != null) {
      availableSunCurves =
          sunECProvider.getProperty("AlgorithmParameters.EC SupportedCurves")
              .split("\\|");
    } else {
  ...
  /**
   * Determines the name of the domain parameters that were used for generating
   * the key.
   *
   * @param key An EC key
  ...
  public static String getNamedCurve(PublicKey publicKey) {
    ...
    ASN1Encodable parameters =
        subjectPublicKeyInfo.getAlgorithm().getParameters();
  ...
  public static boolean isECAvailable(KeyStoreType keyStoreType) {
    return (
        (JavaVersion.getJreVersion().isAtLeast(JavaVersion.JRE_VERSION_170) &&
         sunECProviderAvailable) ||
  ...
  /**
   * Checks if the given named curve is known by the provider backing the
   * KeyStoreType.
   *
   ...
   */
  public static boolean isCurveAvailable(String curveName,
  ...
  /**
   * Converts PKCS#8 EC private key (RFC 5208/5958 ASN.1 PrivateKeyInfo
   * structure) to "traditional" OpenSSL ASN.1 structure ECPrivateKey from RFC
   * 5915. As ECPrivateKey is already in the PrivateKey field of PrivateKeyInfo,
   ...
   * @throws IOException When ECPrivateKey structure in PrivateKeyInfo's
   *     PrivateKey field cannot be parsed
   */
  public static org.bouncycastle.asn1.sec.ECPrivateKey
  convertToECPrivateKeyStructure(ECPrivateKey ecPrivateKey) throws IOException {
    PrivateKeyInfo privateKeyInfo =
        PrivateKeyInfo.getInstance(ecPrivateKey.getEncoded());
  ...
  /**
   * Detect which one of the two EdDSA curves (Ed25519 or Ed448) the given
   * privateKey is.
   *
