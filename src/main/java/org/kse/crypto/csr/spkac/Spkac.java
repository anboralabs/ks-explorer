...
import static org.kse.crypto.csr.spkac.SpkacSubject.*;
...
public class Spkac {
  ...
  private static ResourceBundle res =
      ResourceBundle.getBundle("org/kse/crypto/csr/spkac/resources");
   ...
   */
  public Spkac(String challenge, SignatureType signatureAlgorithm,
               SpkacSubject subject, PublicKey publicKey, PrivateKey privateKey)
  ...
  public Spkac(byte[] data) throws IOException, SpkacException {
    ...
    if (!properties.containsKey(SPKAC_PROPERTY)) {
      throw new SpkacMissingPropertyException(MessageFormat.format(
          res.getString("SpkacNoIncludeRequiredProperty.exception.message"),
          SPKAC_PROPERTY));
    }
  ...
  private Properties readProperties(byte[] data) throws IOException {
    // Properties are defined as name=value pairs where value may be over
    // several lines
      ...
      while ((line = br.readLine()) != null) {
        ...
        if ((equalsPos > 0) && ((equalsPos + 1) < line.length())) {
          ...
          properties.setProperty(
              lastName, String.valueOf(properties.get(lastName)) + line);
        }
      }
    }
    return properties;
  ...
  private SpkacSubject getSubject(Properties properties)
    ...
    if ((cn == null) && (ou == null) && (o == null) && (l == null) &&
        (st == null) && (c == null)) {
      throw new SpkacMissingPropertyException(
          res.getString("SpkacNoIncludeSubjectProperties.exception.message"));
    }
  ...
  private void decodeSpkac(byte[] der) throws SpkacException {
    try {
      ...
      ASN1Sequence publicKeyAndChallenge =
          (ASN1Sequence)signedPublicKeyAndChallenge.getObjectAt(0);
      ASN1Sequence signatureAlgorithm =
          (ASN1Sequence)signedPublicKeyAndChallenge.getObjectAt(1);
      DERBitString signature =
          (DERBitString)signedPublicKeyAndChallenge.getObjectAt(2);

      ASN1ObjectIdentifier signatureAlgorithmOid =
          (ASN1ObjectIdentifier)signatureAlgorithm.getObjectAt(0);
      if (signatureAlgorithm.size() > 1) {
        this.signatureAlgorithmParameters =
            signatureAlgorithm.getObjectAt(1).toASN1Primitive().getEncoded();
      }

      ASN1Sequence spki = (ASN1Sequence)publicKeyAndChallenge.getObjectAt(0);
      DERIA5String challenge =
          (DERIA5String)publicKeyAndChallenge.getObjectAt(1);

      ASN1Sequence publicKeyAlgorithm = (ASN1Sequence)spki.getObjectAt(0);
      DERBitString publicKey = (DERBitString)spki.getObjectAt(1);

      ASN1ObjectIdentifier publicKeyAlgorithmOid =
          (ASN1ObjectIdentifier)publicKeyAlgorithm.getObjectAt(0);
      ASN1Primitive algorithmParameters =
          publicKeyAlgorithm.getObjectAt(1).toASN1Primitive();

      this.challenge = challenge.getString();
      this.publicKey = decodePublicKeyFromBitString(
      ...
      throw new SpkacException(res.getString("NoDecodeSpkac.exception.message"),
                               ex);
    }
  }

  private SignatureType
  getSignatureAlgorithm(ASN1ObjectIdentifier signatureAlgorithmOid)
      throws SpkacException {
    SignatureType sigAlg = SignatureType.resolveOid(
        signatureAlgorithmOid.getId(), signatureAlgorithmParameters);

    if (sigAlg == null) {
      throw new SpkacException(MessageFormat.format(
          res.getString("NoSupportSignatureAlgorithm.exception.message"),
  ...
  private PublicKey
  decodePublicKeyFromBitString(ASN1ObjectIdentifier publicKeyAlgorithmOid,
                               ASN1Primitive algorithmParameters,
                               DERBitString publicKey) throws SpkacException {
    if (publicKeyAlgorithmOid.getId().equals(RSA.oid())) {
      return decodeRsaPublicKeyFromBitString(
          publicKey); // Algorithm parameters are ASN1Null and unnecessary
      ...
      throw new SpkacException(MessageFormat.format(
          res.getString("NoSupportPublicKeyAlgorithm.exception.message"),
          publicKeyAlgorithmOid.getId()));
    }
  }

  private RSAPublicKey decodeRsaPublicKeyFromBitString(DERBitString der)
      throws SpkacException {
    try {
      ASN1Sequence rsaPublicKey = ASN1Sequence.getInstance(der.getBytes());

      BigInteger modulus =
          ((ASN1Integer)rsaPublicKey.getObjectAt(0)).getValue();
      BigInteger publicExponent =
          ((ASN1Integer)rsaPublicKey.getObjectAt(1)).getValue();

      KeyFactory keyFact = KeyFactory.getInstance("RSA");

      return (RSAPublicKey)keyFact.generatePublic(
          new RSAPublicKeySpec(modulus, publicExponent));
    } catch (Exception ex) {
      throw new SpkacException(
          res.getString("NoGenerateRsaPublicKeyFromSpkac.exception.message"),
          ex);
    }
  }
  ...
  private byte[] encodeRsaPublicKeyAsBitString(RSAPublicKey rsaPublicKey)
    ...
    try {
      ...
      throw new SpkacException(
          res.getString("NoEncodeRsaPublicKey.exception.message"), ex);
    }
  }

  private DSAPublicKey
  decodeDsaPublicKeyFromBitString(DERBitString der, BigInteger p, BigInteger q,
                                  BigInteger g) throws SpkacException {
    try {
      BigInteger y = ASN1Integer.getInstance(der.getBytes()).getValue();

      KeyFactory keyFact = KeyFactory.getInstance("DSA");

      return (DSAPublicKey)keyFact.generatePublic(
          new DSAPublicKeySpec(y, p, q, g));
    } catch (Exception ex) {
      throw new SpkacException(
          res.getString("NoGenerateDsaPublicKeyFromSpkac.exception.message"),
          ex);
    }
  }
  ...
  private byte[] encodeDsaPublicKeyAsBitString(DSAPublicKey dsaPublicKey)
    ...
    try {
      ...
      throw new SpkacException(
          res.getString("NoEncodeDsaPublicKey.exception.message"), ex);
    }
  }
    ...
    try {
      ...
      outputProperty(osw, SPKAC_PROPERTY,
                     new String(Base64.encode(
                         createSignedPublicKeyAndChallenge().getEncoded(
                             ASN1Encoding.DER))));
      ...
      throw new SpkacException(res.getString("NoOutputSpkac.exception.message"),
                               ex);
    } finally {
  ...
  private void outputProperty(Writer w, String name, String value)
      throws IOException {
    if (value != null) {
  ...
  public boolean verify() throws SpkacException {
    try {
      ...
      throw new SpkacException(
          res.getString("NoVerifySpkacSignature.exception.message"), ex);
    }
  }
  ...
  private byte[] createSignature(PrivateKey privateKey) throws SpkacException {
    try {
      ...
      throw new SpkacException(
          res.getString("NoCreateSpkacSignature.exception.message"), ex);
    }
  }
  ...
  private byte[] createPublicKeyAndChallengeForSigning() throws SpkacException {
    try {
      return new DERBitString(
                 createPublicKeyAndChallenge().getEncoded(ASN1Encoding.DER))
          .getBytes();
    } catch (Exception ex) {
      throw new SpkacException(
          res.getString(
              "NoGetPublicKeyAndChallengeForSignature.exception.message"),
          ex);
    }
  }
   ...
   * @return Subject
   */
  public SpkacSubject getSubject() { return subject; }
   ...
   */
  public SignatureType getSignatureAlgorithm() { return signatureAlgorithm; }
  ...
  public byte[] getEncoded() { return derSpkac; }

  @Override
  public String toString() {
    return "challenge=" + getChallenge() +
        ", signatureAlgorithm=" + signatureAlgorithm +
        ", signature=" + HexUtil.getHexString(signature) +
        ", subject=" + getSubject() + ", publicKey=" + publicKey;
  }
}
