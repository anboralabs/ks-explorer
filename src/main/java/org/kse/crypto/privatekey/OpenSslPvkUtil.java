...
/**
 * Provides utility methods relating to OpenSSL (= PKCS#1 for RSA) encoded
 * private keys.
 *
 * <pre>
...
public class OpenSslPvkUtil {
    ...
    try {
      ...
      if (privateKey instanceof RSAPrivateCrtKey) {
        ...
        org.bouncycastle.asn1.sec.ECPrivateKey ecPrivateKey =
            EccUtil.convertToECPrivateKeyStructure(ecKey);
      ...
      throw new CryptoException(
          res.getString("NoDerEncodeOpenSslPrivateKey.exception.message"), ex);
    }
  }
   ...
   *     private key
   */
  public static String getEncrypted(PrivateKey privateKey,
                                    OpenSslPbeType pbeType, Password password)
    ...
    try {
      byte[] encryptKey =
          deriveKeyFromPassword(password, salt, pbeType.keySize());

      // Create cipher - use all of the salt as the IV
      Cipher cipher =
          createCipher(pbeType.jceCipher(), encryptKey, salt, ENCRYPT_MODE);

      encOpenSsl = cipher.doFinal(openSsl);
    } catch (GeneralSecurityException ex) {
      throw new CryptoException(
          MessageFormat.format(
              res.getString("OpenSslEncryptionFailed.exception.message"),
              pbeType.friendly()),
          ex);
    }
    ...
    String dekInfoAttrValue = MessageFormat.format(DEK_INFO_ATTR_VALUE_TEMPLATE,
                                                   pbeType.dekInfo(), saltHex);
   ...
   */
  public static PrivateKey load(byte[] pvkData)
    ...
    if (encType == null) {
      throw new CryptoException(
          res.getString("NotValidOpenSsl.exception.message"));
    }

    if (encType == ENCRYPTED) {
      throw new PrivateKeyEncryptedException(
          res.getString("OpenSslIsEncrypted.exception.message"));
    }
      ...
      if (openSsl instanceof ASN1Sequence) {
        ...
        if (seq.size() == 9) { // RSA private key
          ...
          BigInteger privateExponent =
              ((ASN1Integer)seq.getObjectAt(3)).getValue();
          BigInteger primeP = ((ASN1Integer)seq.getObjectAt(4)).getValue();
          BigInteger primeQ = ((ASN1Integer)seq.getObjectAt(5)).getValue();
          BigInteger primeExponentP =
              ((ASN1Integer)seq.getObjectAt(6)).getValue();
          BigInteger primeExponenetQ =
              ((ASN1Integer)seq.getObjectAt(7)).getValue();
          BigInteger crtCoefficient =
              ((ASN1Integer)seq.getObjectAt(8)).getValue();

          if (!version.equals(VERSION)) {
            throw new CryptoException(MessageFormat.format(
                res.getString("OpenSslVersionIncorrect.exception.message"),
          ...
          RSAPrivateCrtKeySpec rsaPrivateCrtKeySpec = new RSAPrivateCrtKeySpec(
              ...
              primeExponentP, primeExponenetQ, crtCoefficient);
          ...
          BigInteger secretExponentX =
              ((ASN1Integer)seq.getObjectAt(5)).getValue();
          ...
          DSAPrivateKeySpec dsaPrivateKeySpec = new DSAPrivateKeySpec(
              secretExponentX, primeModulusP, primeQ, generatorG);

          KeyFactory keyFactory = KeyFactory.getInstance("DSA");
          return keyFactory.generatePrivate(dsaPrivateKeySpec);
        } else if (seq.size() >= 2) { // EC private key (RFC 5915)

          org.bouncycastle.asn1.sec.ECPrivateKey pKey =
              org.bouncycastle.asn1.sec.ECPrivateKey.getInstance(seq);
          AlgorithmIdentifier algId = new AlgorithmIdentifier(
              X9ObjectIdentifiers.id_ecPublicKey, pKey.getParametersObject());
          PrivateKeyInfo privInfo = new PrivateKeyInfo(algId, pKey);
          return new JcaPEMKeyConverter().getPrivateKey(privInfo);
        } else {
          throw new CryptoException(MessageFormat.format(
              res.getString("OpenSslSequenceIncorrectSize.exception.message"),
              "" + seq.size()));
        }
      } else {
        throw new CryptoException(
            res.getString("OpenSslSequenceNotFound.exception.message"));
      }
    } catch (Exception ex) {
      throw new CryptoException(
          res.getString("NoLoadOpenSslPrivateKey.exception.message"), ex);
    }
  }
   ...
   * @throws PrivateKeyPbeNotSupportedException If private key PBE algorithm is
   *     not supported
   * @throws CryptoException                    Problem encountered while
   *     loading the private key
   * @throws IOException                        An I/O error occurred
   */
  public static PrivateKey loadEncrypted(byte[] pvkData, Password password)
      throws CryptoException, IOException {
    ...
    if (encType == UNENCRYPTED) {
      throw new PrivateKeyUnencryptedException(
          res.getString("OpenSslIsUnencrypted.exception.message"));
    }
    ...
    if (separator == -1) {
      throw new CryptoException(MessageFormat.format(
          res.getString("OpenSslDekInfoMalformed.exception.message"), dekInfo));
    }
    ...
    if (pbeType == null) {
      throw new PrivateKeyPbeNotSupportedException(
          encAlg, MessageFormat.format(
                      res.getString(
                          "PrivateKeyWrappingAlgUnsupported.exception.message"),
                      encAlg));
    }

    try {
      byte[] decryptKey =
          deriveKeyFromPassword(password, saltBytes, pbeType.keySize());

      // Create cipher - use all of the salt as the IV
      Cipher cipher = createCipher(pbeType.jceCipher(), decryptKey, saltBytes,
                                   DECRYPT_MODE);
      ...
      throw new CryptoException(
          MessageFormat.format(
              res.getString("OpenSslDecryptionFailed.exception.message"),
              pbeType.friendly()),
          ex);
    }
  }
   ...
   */
  public static EncryptionType getEncryptionType(byte[] openSsl)
      throws IOException {
    ...
    if (pemInfo != null) {
      ...
      // PEM type of OpenSSL?
      if (OPENSSL_RSA_PVK_PEM_TYPE.equals(pemType) ||
          OPENSSL_DSA_PVK_PEM_TYPE.equals(pemType) ||
        ...
        if ((pemAttributes != null) &&
            (pemAttributes.get(PROC_TYPE_ATTR_NAME) != null) &&
            (pemAttributes.get(PROC_TYPE_ATTR_NAME)
                 .getValue()
                 .equals(PROC_TYPE_ATTR_VALUE)) &&
    ...
    // In ASN.1 format?
    try {
      // If OpenSSL will be a sequence of 9 (RSA) or 6 (DSA) integers or 2-4
      // mixed elements (EC)
  ...
  private static byte[] deriveKeyFromPassword(Password password, byte[] salt,
                                              int keySize)
      throws CryptoException {
  ...
  private static Cipher createCipher(String transformation, byte[] key,
                                     byte[] iv, int operation)
      throws CryptoException {
    ...
    try {
      ...
      throw new CryptoException(
          MessageFormat.format(
              res.getString("OpenSslCreateCipherFailed.exception.message"),
              transformation),
          ex);
    }
  }
