...
public class PacProxySelector extends ProxySelector {
  private static final ResourceBundle res =
      ResourceBundle.getBundle("org/kse/utilities/net/resources");
  ...
  public PacProxySelector(URI pacURI) {
    ...
    // As load and compile of pac scripts is time-consuming we do this on first
    // call to select
  }
   ...
   * @return List of proxies; if there are any issues with the PAC returns 'no
   *     proxy'
   */
  @Override
  public List<Proxy> select(URI uri) {
    ...
    try {
      pacFunctionReturn = (String)pacScript.invokeFunction(
          "FindProxyForURL", uri.toString(), uri.getHost());
    } catch (Exception ex) {
      ex.printStackTrace();
      return singletonList(Proxy.NO_PROXY);
    }
  ...
  private String loadPacScript(URI pacURI) throws PacProxyException {
    ...
    try {
      // ...and set use of no proxy selector. We don't want to try and use any
      // proxy to get the the pac script
      ...
      throw new PacProxyException(
          MessageFormat.format(
              res.getString("NoLoadPacScript.exception.message"), pacURI),
          ex);
    } finally {
  ...
  private Invocable compilePacScript(String pacScript)
      throws PacProxyException {
    try {
      ScriptEngine jsEngine;

      // Nashorn was removed in Java 15, the standalone Nashorn uses different
      // packages and is compiled for Java 11
      // TODO remove this when support for Java 8 is dropped
      if (JavaVersion.getJreVersion().isAtLeast(JavaVersion.JRE_VERSION_15)) {
        jsEngine = getScriptEngine(
      ...
      jsEngine.put(
          "dnsDomainIs",
          (BiFunction<String, String, Boolean>)PacHelperFunctions::dnsDomainIs);
      jsEngine.put(
          "dnsDomainLevels",
          (Function<String, Integer>)PacHelperFunctions::dnsDomainLevels);
      jsEngine.put("dnsResolve",
                   (Function<String, String>)PacHelperFunctions::dnsResolve);
      jsEngine.put("isResolvable",
                   (Function<String, Boolean>)PacHelperFunctions::isResolvable);
      jsEngine.put("myIpAddress",
                   (Supplier<String>)PacHelperFunctions::myIpAddress);
      jsEngine.put(
          "isPlainHostName",
          (Function<String, Boolean>)PacHelperFunctions::isPlainHostName);
      jsEngine.put("localHostOrDomainIs",
                   (BiFunction<String, String, Boolean>)
                       PacHelperFunctions::localHostOrDomainIs);
      jsEngine.put(
          "shExpMatch",
          (BiFunction<String, String, Boolean>)PacHelperFunctions::shExpMatch);
      jsEngine.put("isInNet", (TriFunction<String, String, String, Boolean>)
                                  PacHelperFunctions::isInNet);
      jsEngine.put("dateRange",
                   (VarFunction<Object, Boolean>)PacHelperFunctions::dateRange);
      jsEngine.put(
          "weekdayRange",
          (VarFunction<Object, Boolean>)PacHelperFunctions::weekdayRange);
      jsEngine.put("timeRange",
                   (VarFunction<Object, Boolean>)PacHelperFunctions::timeRange);
      ...
      throw new PacProxyException(
          res.getString("NoCompilePacScript.exception.message"), ex);
    }
  }

  private ScriptEngine getScriptEngine(String nashornScriptEngineFactoryClass,
                                       String classFilterClass) {
    // NashornScriptEngineFactory nashornScriptEngineFactory = new
    // NashornScriptEngineFactory(); ScriptEngine jsEngine =
    // nashornScriptEngineFactory.getScriptEngine(new PacClassFilter());
    try {
      Class<?> nashornScriptEngineFactory =
          Class.forName(nashornScriptEngineFactoryClass);
      Class<?> classFilter = Class.forName(classFilterClass);

      Object classFilterProxy = java.lang.reflect.Proxy.newProxyInstance(
          PacProxySelector.class.getClassLoader(), new Class[] {classFilter},
          (proxy, method, args) -> {
      ...
      Method getScriptEngine =
          nashornScriptEngineFactory.getMethod("getScriptEngine", classFilter);
      return (ScriptEngine)getScriptEngine.invoke(
          nashornScriptEngineFactory.newInstance(), classFilterProxy);
   ...
   *
   * @param uri           The URI that the proxy at socketAddress failed to
   *     serve
   ...
   * @return PAC URI
   */
  public URI getPacURI() { return pacURI; }
