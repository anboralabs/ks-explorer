...
public class SslUtils {
  ...
  private static ResourceBundle res =
      ResourceBundle.getBundle("org/kse/utilities/ssl/resources");
   ...
   */
  public static SslConnectionInfos readSSLConnectionInfos(String host, int port,
                                                          KeyStore keyStore,
                                                          char[] password)
      throws CryptoException, IOException {

    URL url =
        new URL(MessageFormat.format("https://{0}:{1}/", host, "" + port));
    ...
    try {
      ...
      // We are only interested in getting the SSL certificates even if they are
      // invalid either in and of themselves or for the host name they are
      // associated with

      // 1) set connection's SSL Socket factory to have a very trusting trust
      // manager
      ...
      // register our handshake completed listener in order to retrieve SSL
      // connection infos later
      SSLSocketFactory factory = context.getSocketFactory();
      RetrieveSslInfosHandshakeListener handshakeListener =
          new RetrieveSslInfosHandshakeListener();
      boolean sniEnabled = true;
      connection.setSSLSocketFactory(
          new CustomSslSocketFactory(factory, handshakeListener, sniEnabled));
      ...
      try {
        ...
        if (e.getMessage().contains("unrecognized_name")) {
          ...
          connection.setSSLSocketFactory(new CustomSslSocketFactory(
              factory, handshakeListener, sniEnabled));
          connection.connect();
        } else {
          throw e;
        }
      ...
      // this is necessary in order to cause a handshake exception when the
      // client cert is not accepted
      if (keyStore != null) {
        connection.getResponseMessage();
      }

      SslConnectionInfos sslConnectionInfos =
          handshakeListener.getSslConnectionInfos();
