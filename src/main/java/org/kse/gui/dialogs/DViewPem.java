...
public class DViewPem extends JEscDialog {
   ...
   * @throws CryptoException A problem was encountered getting the certificate's
   *     PEM'd DER
   *                         encoding
   */
  ...
  private void initComponents() throws CryptoException {
    ...
    PlatformUtil.setMnemonic(
        jbCopy, res.getString("DViewPem.jbCopy.mnemonic").charAt(0));
    jbCopy.setToolTipText(res.getString("DViewPem.jbCopy.tooltip"));

    jbCopy.addActionListener(evt -> copyPressed());

    jbExport = new JButton(res.getString("DViewPem.jbExport.text"));
    PlatformUtil.setMnemonic(
        jbExport, res.getString("DViewPem.jbExport.mnemonic").charAt(0));
    ...
    jpButtons = PlatformUtil.createDialogButtonPanel(
        jbOK, null, new JButton[] {jbCopy, jbExport});
    ...
    jspPem = PlatformUtil.createScrollPane(
        jtaPem, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
  ...
  res.getString("DViewPem.ChooseExportFile.button"));
          ...
          if (chosenFile.isFile()) {
              String message =
  MessageFormat.format(res.getString("DViewPem.OverWriteFile.message"),
  chosenFile);

              int selected = JOptionPane.showConfirmDialog(this, message, title,
  JOptionPane.YES_NO_OPTION); if (selected != JOptionPane.YES_OPTION) { return;
              }
          }
  ...
  MessageFormat.format(res.getString("DViewPem.NoWriteFile.message"),
                                                                   chosenFile),
  title, JOptionPane.WARNING_MESSAGE); return; } catch (Exception ex) {
      ...
      JOptionPane.showMessageDialog(this,
  res.getString("DViewPem.ExportPemCertificateSuccessful.message"), title,
