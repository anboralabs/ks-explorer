...
public class JCertificateFingerprint extends JPanel {
   ...
   */
  public JCertificateFingerprint(Project project, int columns) {
  ...
  private void initComponents(int columns) {
    ...
    jcbFingerprintAlg.setToolTipText(
        res.getString("JCertificateFingerprint.jcbFingerprintAlg.tooltip"));
    ...
    jtfCertificateFingerprint.setToolTipText(res.getString(
        "JCertificateFingerprint.jtfCertificateFingerprint.tooltip"));

    ImageIcon viewIcon = new ImageIcon(
        getClass().getResource("images/view_cert_fingerprint.png"));
    jbViewCertificateFingerprint = new JButton(viewIcon);

    jbViewCertificateFingerprint.setToolTipText(res.getString(
        "JCertificateFingerprint.jbViewCertificateFingerprint.tooltip"));
  ...
  private void populateFingerprint() {
    if (encodedCertificate != null) {
      DigestType fingerprintAlg =
          (DigestType)jcbFingerprintAlg.getSelectedItem();

      try {
        jtfCertificateFingerprint.setText(DigestUtil.getFriendlyMessageDigest(
            encodedCertificate, fingerprintAlg));
  ...
  private void displayFingerprint() {
    ...
    DigestType fingerprintAlg = (DigestType)
    jcbFingerprintAlg.getSelectedItem();

    if (container instanceof JDialog) {
        DViewCertificateFingerprint dViewCertificateFingerprint = new
    DViewCertificateFingerprint( (JDialog) container, encodedCertificate,
    fingerprintAlg);
        dViewCertificateFingerprint.setLocationRelativeTo(container);
        dViewCertificateFingerprint.setVisible(true);
    } else if (container instanceof JFrame) {
        DViewCertificateFingerprint dViewCertificateFingerprint = new
    DViewCertificateFingerprint( (JFrame) container, encodedCertificate,
    fingerprintAlg);
        dViewCertificateFingerprint.setLocationRelativeTo(container);
        dViewCertificateFingerprint.setVisible(true);
    }*/
