...
public class HexUtil {
  ...
  /**
   * Get hex string for the supplied big integer: "<prefix><hex string>" where
   * hex string is optionally output in groups of n characters subdivided by
   * spaces.
   *
   * @param bigInt Big integer
   * @param groupSize Size of char groups separated by spaces (use 0 to
   *     disable), 0-padding is added if necessary
   * @param prefix Prefix to put before actual hex data
   * @param maxLineLength Add new line when max line length is reached; "0"
   *     means no line breaks
   * @return Hex string
   */
  public static String getHexString(BigInteger bigInt, String prefix,
                                    int groupSize, int maxLineLength) {
   ...
   * @return Hex string
   */
  public static String getHexString(byte[] bytes, String prefix, int groupSize,
                                    int maxLineLength) {
    ...
    // Insert any required padding to get groups of exactly "groupSize"
    // characters
    ...
    // Output with leading prefix (usually "0x"), spaces to form groups and line
    // breaks
  ...
  public static String getHexStringWithSep(byte[] data, char separator) {
    ...
    StringBuilder strBuff =
        new StringBuilder(Hex.toHexString(data).toUpperCase());
