...
public class KeyStoreState {
  ...
  protected static ResourceBundle res =
      ResourceBundle.getBundle("org/kse/utilities/history/resources");
   ...
   */
  public void setAsCurrentState() { history.setCurrentState(this); }
   ...
   */
  public boolean isInitialState() { return this == history.getInitialState(); }
   ...
   * @return True if it is
   */
  public boolean isSavedState() { return this == history.getSavedState(); }

  /**
   * Set this state to be the saved state in the history.
   */
  public void setAsSavedState() { history.setSavedState(this); }

  /**
   * Set the previous state as the current state if it exists.
   *
   * @throws CryptoException If entry passwords could not be propogated to new
   *     state
   */
   ...
   */
  public boolean hasPreviousState() { return previous != null; }
   ...
   */
  public KeyStoreState previousState() { return previous; }
   ...
   * @return True if there is
   */
  public boolean hasNextState() { return next != null; }
   ...
   */
  public KeyStoreState nextState() { return next; }
  ...
  protected void propagateNewPasswords(KeyStoreState targetState)
    ...
    // Copy all entry passwords not found in the target state from the current
    ...
    try {
      ...
      throw new CryptoException(
          res.getString("NoPropagateEntryPasswords.exception.message"), ex);
    }
  }

  protected boolean isPasswordPropagationValid(KeyStoreState targetState,
                                               String alias, Password password)
      throws GeneralSecurityException {
    // A password should only be propagated to a target state if it is correct
    // and represents the same private key as the current state
    return isEntryPasswordCorrect(targetState, alias, password) &&
        isEntryPrivateKeyEqual(targetState, alias, password);
  }

  protected boolean isEntryPasswordCorrect(KeyStoreState targetState,
                                           String alias, Password password) {
    try {
  ...
  protected boolean isEntryPrivateKeyEqual(KeyStoreState targetState,
                                           String alias, Password password)
      throws GeneralSecurityException {
    Key currentKey = keyStore.getKey(alias, password.toCharArray());
    Key targetKey =
        targetState.getKeyStore().getKey(alias, password.toCharArray());

    // JDKDSAPrivateKey has no equals method defined
    if ((currentKey instanceof JDKDSAPrivateKey) ||
        (targetKey instanceof JDKDSAPrivateKey)) {
      ...
      return currentDsaKey.getX().equals(targetDsaKey.getX()) &&
          currentDsaKey.getParams().getG().equals(
              targetDsaKey.getParams().getG()) &&
          currentDsaKey.getParams().getP().equals(
              targetDsaKey.getParams().getP()) &&
          currentDsaKey.getParams().getQ().equals(
              targetDsaKey.getParams().getQ());
    } else {
