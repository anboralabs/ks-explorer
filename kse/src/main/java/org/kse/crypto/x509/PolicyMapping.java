...
/**
 * Helper class that represents single <code>PolicyMapping</code> objects, that
 * are used in KSE because binor had them. <p> RFC 5280 and BC only know
 * <code>PolicyMappings</code> (= sequence of <code>PolicyMapping</code>): <pre>
...
public class PolicyMapping extends ASN1Object {
   ...
   */
  public PolicyMapping(ASN1ObjectIdentifier issuerDomainPolicy,
  ...
  private PolicyMapping(ASN1Sequence seq) {
    // java object in sequence is actually not ASN1ObjectIdentifier but
    // CertPolicyId, so we do a conversion in order to avoid possible class cast
    // exception here
    this.issuerDomainPolicy =
        ASN1ObjectIdentifier.getInstance(seq.getObjectAt(0).toASN1Primitive());
    this.subjectDomainPolicy =
        ASN1ObjectIdentifier.getInstance(seq.getObjectAt(1).toASN1Primitive());
  }
  ...
  public boolean equals(Object paramObject) {
    ...
    try {
      return Arrays.equals(((ASN1Object)paramObject).getEncoded(),
                           getEncoded());
    } catch (Exception e) {
      // ignore
    }
