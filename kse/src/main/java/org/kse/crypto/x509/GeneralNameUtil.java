...
public class GeneralNameUtil {
  ...
  public static String safeToString(GeneralName generalName,
    ...
    case GeneralName.directoryName:
      ...
      return MessageFormat.format(
          res.getString("GeneralNameUtil.DirectoryGeneralName"),
          directoryName.toString());
    case GeneralName.dNSName:
      DERIA5String dnsName = (DERIA5String)generalName.getName();

      return MessageFormat.format(
          res.getString("GeneralNameUtil.DnsGeneralName"), dnsName.getString());
    case GeneralName.iPAddress:
      String ipAddressString = parseIpAddress(generalName);
      return MessageFormat.format(
          res.getString("GeneralNameUtil.IpAddressGeneralName"),
          ipAddressString);
    case GeneralName.registeredID:
      ...
      return MessageFormat.format(
          res.getString("GeneralNameUtil.RegisteredIdGeneralName"),
          ObjectIdUtil.toString(registeredId));
    case GeneralName.rfc822Name:
      DERIA5String rfc822Name = (DERIA5String)generalName.getName();

      return MessageFormat.format(
          res.getString("GeneralNameUtil.Rfc822GeneralName"),
          rfc822Name.getString());
    case GeneralName.uniformResourceIdentifier:
      DERIA5String uri = (DERIA5String)generalName.getName();

      String link = addLinkForURI ? "<a href=\"" + uri.getString() + "\">" +
                                        uri.getString() + "</a>"
                                  : uri.getString();

      return MessageFormat.format(
          res.getString("GeneralNameUtil.UriGeneralName"), link);
  ...
  public static boolean isGeneralNameEmpty(GeneralName generalName) {
    ...
    case GeneralName.iPAddress:
      byte[] ipAddressBytes =
          ((ASN1OctetString)generalName.getName()).getOctets();
  ...
  public static String parseIpAddress(GeneralName generalName) {
    ...
    if (ipAddressBytes.length == 4 || ipAddressBytes.length == 16) {
      try {
        ipAddressString =
            InetAddress.getByAddress(ipAddressBytes).getHostAddress();
      ...
      if (ipAddressBytes.length == 8 || ipAddressBytes.length == 32) {
        ...
        try {
          ipAddressString =
              InetAddress.getByAddress(ipAddress).getHostAddress();
        } catch (UnknownHostException e) {
          // length of IP address has been checked before
        }
        ...
        try {
          String netmaskString =
              InetAddress.getByAddress(netMask).getHostAddress();
  ...
  public static String parseUPN(GeneralName generalName) {
    ...
    if (UPN_OID.equals(oid.getId())) {
      ASN1TaggedObject asn1TaggedObject =
          (ASN1TaggedObject)otherName.getObjectAt(1);
      ASN1UTF8String upn =
          ASN1UTF8String.getInstance(asn1TaggedObject.getBaseObject());
      return MessageFormat.format(
          res.getString("GeneralNameUtil.OtherGeneralName"), "UPN",
          upn.getString());
    }
    ...
    try {
      return MessageFormat.format(
          res.getString("GeneralNameUtil.OtherGeneralName"),
          ObjectIdUtil.toString(oid),
          HexUtil.getHexString(
  ...
  public static String toString(GeneralName generalName) throws IOException {
    ...
    case GeneralName.ediPartyName:
      ...
      DirectoryString nameAssigner =
          DirectoryString.getInstance(ediPartyName.getObjectAt(0));
      DirectoryString partyName =
          DirectoryString.getInstance(ediPartyName.getObjectAt(1));
      ...
      if (nameAssignerStr != null) {
        return MessageFormat.format(
            res.getString("GeneralNameUtil.EdiPartyGeneralName"),
            nameAssignerStr, partyNameStr);
      } else {
        return MessageFormat.format(
            res.getString("GeneralNameUtil.EdiPartyGeneralNameNoAssigner"),
            partyNameStr);
      }
    case GeneralName.otherName:
      return parseUPN(generalName);
    case GeneralName.x400Address:
       ...
       * The Oracle CertificateFactory blows up if a certificate extension
       * contains this anyway
       */
      ASN1Encodable x400Address = generalName.getName();

      return MessageFormat.format(
          res.getString("GeneralNameUtil.X400AddressGeneralName"),
          HexUtil.getHexString(
