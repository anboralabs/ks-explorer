...
public final class X509CertUtil {
  ...
  public static final String BASE64_TESTER =
      "^([A-Za-z0-9+/]{4})*([A-Za-z0-9+/]{4}|[A-Za-z0-9+/]{3}=|[A-Za-z0-9"
      + "+/]{2}==)$";
   ...
   *     certificate(s)
   */
  public static X509Certificate[] loadCertificates(byte[] certsBytes)
  ...
  private static List<X509Certificate> loadAsBase64(byte[] certsBytes,
    ...
    try {
      ...
      Collection<? extends Certificate> certs =
          cf.generateCertificates(new ByteArrayInputStream(base64Decoded));
  ...
  private static X509Certificate[] loadCertificatesPkiPath(InputStream is)
      throws CryptoException {
    try {
      CertificateFactory cf =
          CertificateFactory.getInstance(X509_CERT_TYPE, KSE.BC);
      ...
      throw new CryptoException(
          res.getString("NoLoadPkiPath.exception.message"), e);
    } finally {
      IOUtils.closeQuietly(is);
    }
  }

  private static List<X509Certificate> loadAsPEM(byte[] bytes,
                                                 CertificateFactory cf) {

    PEMParser pemParser = new PEMParser(new StringReader(new String(bytes)));
    JcaX509CertificateConverter jcaX509CertConverter =
    ...
    try {
      ...
      while (pemObject != null) {
        ...
        if (pemObject instanceof X509CertificateHolder) {
          certs.add(jcaX509CertConverter.getCertificate(
              (X509CertificateHolder)pemObject));
        } else if (pemObject instanceof X509TrustedCertificateBlock) {
          X509TrustedCertificateBlock trustedCertBlock =
              (X509TrustedCertificateBlock)pemObject;
          certs.add(jcaX509CertConverter.getCertificate(
              trustedCertBlock.getCertificateHolder()));
          ...
          if (!certsFromPkcs7.isEmpty()) {
            List<X509Certificate> x509Certificates =
                convertCertificates(certsFromPkcs7);
  ...
  public static X509CRL loadCRL(byte[] crlData) throws CryptoException {
    ...
    if (crlData == null || crlData.length == 0) {
      throw new CryptoException(
          res.getString("NoLoadCrl.exception.message"),
  ...
  convertCertificates(Collection<? extends Certificate> certs)
      throws CryptoException {
  ...
  public static X509Certificate convertCertificate(Certificate certIn)
    ...
    try {
      ...
      throw new CryptoException(
          res.getString("NoConvertCertificate.exception.message"), e);
    }
  }
  ...
  private static X509Certificate findIssuedCert(X509Certificate issuerCert,
                                                X509Certificate[] certs) {
    // Find a certificate issued by the supplied certificate
    for (X509Certificate cert : certs) {
      if (issuerCert.getSubjectX500Principal().equals(
              cert.getSubjectX500Principal()) &&
          issuerCert.getIssuerX500Principal().equals(
  ...
  /**
   * Checks if certificate was issued by the other certificate by checking first
   * the DN and only if the issuer DN matches the subject DN, then the signature
   * is verified. This avoids the slow verification operation when it is
   ...
   */
  public static boolean isIssuedBy(X509Certificate cert,
    ...
    if (issuerCert.getSubjectX500Principal().equals(
      ...
      try {
        ...
        // ignore technical verification issues as they are not relevant for
        // finding chains
        return true;
      }
    }
   ...
   */
  public static byte[] getCertEncodedX509(X509Certificate cert)
      throws CryptoException {
    try {
      return cert.getEncoded();
    } catch (CertificateException ex) {
      throw new CryptoException(
          res.getString("NoDerEncodeCertificate.exception.message"), ex);
    }
  }
   ...
   * @throws CryptoException If there was a problem encoding the certificate
   */
  public static String getCertEncodedX509Pem(X509Certificate cert)
      throws CryptoException {
    PemInfo pemInfo =
        new PemInfo(CERT_PEM_TYPE, null, getCertEncodedX509(cert));
   ...
   */
  public static String getCertsEncodedX509Pem(X509Certificate[] certs)
      throws CryptoException {
  ...
  public static byte[] getCertsEncodedPkcs7(X509Certificate[] certs)
    ...
    try {
      ...
      throw new CryptoException(
          res.getString("NoPkcs7Encode.exception.message"), e);
    }
  }
   ...
   * @throws CryptoException If there was a problem encoding the certificate
   */
  public static String getCertEncodedPkcs7Pem(X509Certificate cert)
      throws CryptoException {
   ...
   * @throws CryptoException If there was a problem encoding the certificates
   */
  public static String getCertsEncodedPkcs7Pem(X509Certificate[] certs)
      throws CryptoException {
    PemInfo pemInfo =
        new PemInfo(PKCS7_PEM_TYPE, null, getCertsEncodedPkcs7(certs));
    return PemUtil.encode(pemInfo);
  }
  ...
  public static boolean verifyCertificate(X509Certificate signedCert,
    ...
    try {
      ...
      // Problem verifying
      throw new CryptoException(
          res.getString("NoVerifyCertificate.exception.message"), ex);
    }
  }
   ...
   */
  public static X509Certificate[] establishTrust(X509Certificate cert,
                                                 KeyStore[] keyStores)
      throws CryptoException {
  ...
  establishTrust(X509Certificate cert, List<X509Certificate> compCerts)
      throws CryptoException {
    /*
     * Check whether or not a trust path exists between the supplied X.509
    ...
    for (int i = 0; i < compCerts.size(); i++) {
      ...
      if (cert.getIssuerX500Principal().equals(
              compCert.getSubjectX500Principal())) {
        // Verify if the comparison certificate's private key was used to sign
        // the certificate
        if (X509CertUtil.verifyCertificate(cert, compCert)) {
          // If the comparision certificate is self-signed then a chain of trust
          // exists
          if (compCert.getSubjectX500Principal().equals(
                  compCert.getIssuerX500Principal())) {
  ...
  private static List<X509Certificate> extractCertificates(KeyStore keyStore)
    ...
    try {
      ...
      throw new CryptoException(
          res.getString("NoExtractCertificates.exception.message"), ex);
    }
  }
  ...
  public static String matchCertificate(KeyStore keyStore, X509Certificate cert)
    ...
    try {
      ...
      throw new CryptoException(
          res.getString("NoMatchCertificate.exception.message"), ex);
    }
  }
  ...
  public static String getCertificateAlias(X509Certificate cert) {
    ...
    String subjectCn =
        X500NameUtils.extractCN(X500NameUtils.x500PrincipalToX500Name(subject));
    String issuerCn =
        X500NameUtils.extractCN(X500NameUtils.x500PrincipalToX500Name(issuer));
  ...
  public static String getShortName(X509Certificate cert) {
    X500Name subject =
        X500NameUtils.x500PrincipalToX500Name(cert.getSubjectX500Principal());
  ...
  /**
   * Generate certificate serial number with the length configured in the
  ...
  /*public static String generateCertSerialNumber() {
      int snLength =
  PreferencesManager.getPreferences().getSerialNumberLengthInBytes(); return
  HexUtil.getHexString(SerialNumbers.generate(snLength), "0x", 0, 0);
  }*/
