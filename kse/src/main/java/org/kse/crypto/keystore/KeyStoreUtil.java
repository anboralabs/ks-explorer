...
import static org.kse.crypto.keystore.KeyStoreType.*;
...
public final class KeyStoreUtil {
   ...
   */
  public static KeyStore create(KeyStoreType keyStoreType)
      throws CryptoException, IOException {
    if (!keyStoreType.isFileBased()) {
      throw new CryptoException(MessageFormat.format(
          res.getString("NoCreateKeyStoreNotFile.exception.message"),
          keyStoreType.jce()));
    }
    ...
    try {
      ...
      throw new CryptoException(
          MessageFormat.format(
              res.getString("NoLoadKeyStoreType.exception.message"),
              keyStoreType),
          ex);
    }
   ...
   *         recognised type
   * @throws KeyStoreLoadException Problem encountered loading the KeyStore as
   *     the auto-detected
   *                               type
  ...
  public static KeyStore load(File keyStoreFile, Password password)
    ...
    try {
      ...
      throw new CryptoException(
          res.getString("NoLoadKeyStore.exception.message"), ex);
    }
  ...
  public static KeyStore load(File keyStoreFile, Password password,
    ...
    if (!keyStoreType.isFileBased()) {
      throw new CryptoException(MessageFormat.format(
          res.getString("NoLoadKeyStoreNotFile.exception.message"),
          keyStoreType.jce()));
    }
    ...
    try (FileInputStream fis = new FileInputStream(keyStoreFile)) {
      if (password.isEmpty() && (keyStoreType == KeyStoreType.JKS ||
  ...
  public static KeyStore loadAppleKeychain() throws CryptoException {
    if (!isAppleKeychainSupported()) {
      throw new CryptoException(
          res.getString("AppleKeychainNotSupported.exception.message"));
    }
    ...
    try {
      ...
      throw new CryptoException(
          MessageFormat.format(
              res.getString("NoCreateKeyStore.exception.message"),
              KEYCHAIN.jce()),
          ex);
    }
   ...
   * @throws CryptoException Problem encountered loading the KeyStore
   */
  public static KeyStore loadMsCapiStore(MsCapiStoreType msCapiStoreType)
    ...
    if (!areMsCapiStoresSupported()) {
      ...
      throw new CryptoException(
          res.getString("MsCapiStoresNotSupported.exception.message"));
    }
   ...
   *                               regular file, does not exist but cannot be
   * created, or cannot be opened for any other reason
   * @throws IOException           An I/O error occurred
   */
  public static void save(KeyStore keyStore, File keyStoreFile,
                          Password password)
      throws CryptoException, IOException {
    ...
    if (!keyStoreType.isFileBased()) {
      throw new CryptoException(MessageFormat.format(
          res.getString("NoSaveKeyStoreNotFile.exception.message"),
          keyStoreType.jce()));
    }
    ...
    try (FileOutputStream fos = new FileOutputStream(keyStoreFile)) {
      ...
      throw new CryptoException(
          res.getString("NoSaveKeyStore.exception.message"), ex);
    }
  }
  ...
  public static boolean containsKey(KeyStore keyStore) throws CryptoException {
    try {
      ...
      throw new CryptoException(
          res.getString("NoCheckKeyStoreKeys.exception.message"), ex);
    }
  }
   ...
   */
  public static boolean isKeyPairEntry(String alias, KeyStore keyStore)
   ...
   * @throws KeyStoreException If there was a problem accessing the KeyStore.
   */
  public static boolean isKeyEntry(String alias, KeyStore keyStore)
      throws KeyStoreException {
    return keyStore.isKeyEntry(alias) &&
        (keyStore.getCertificateChain(alias) == null ||
         keyStore.getCertificateChain(alias).length == 0);
   ...
   * @throws KeyStoreException If there was a problem accessing the KeyStore.
   */
  public static boolean isTrustedCertificateEntry(String alias,
                                                  KeyStore keyStore)
      throws KeyStoreException {
  ...
  public static KeyStore copy(KeyStore keyStore) throws CryptoException {
    ...
    try {
      ...
      KeyStore theCopy =
          KeyStoreUtil.create(KeyStoreType.resolveJce(keyStore.getType()));
      theCopy.load(new ByteArrayInputStream(baos.toByteArray()), emptyPassword);

      return theCopy;
    } catch (CryptoException | GeneralSecurityException |
             IllegalStateException | IOException ex) {
      throw new CryptoException(
          res.getString("NoCopyKeyStore.exception.message"), ex);
    }
  }

  private static KeyStore getKeyStoreInstance(KeyStoreType keyStoreType)
      throws CryptoException {
    try {
      if (keyStoreType == BKS || keyStoreType == UBER ||
          keyStoreType == BCFKS) {
        if (Security.getProvider(BOUNCY_CASTLE.jce()) == null) {
          throw new CryptoException(MessageFormat.format(
              res.getString("NoProvider.exception.message"),
              BOUNCY_CASTLE.jce()));
        }
   ...
   * @throws KeyStoreException If there was a problem accessing the KeyStore.
   */
  public static boolean isECKeyPair(String alias, KeyStore keyStore)
      throws KeyStoreException {
  ...
  /**
   * Is the keystore entry type supported (e.g. exclude Card Verifiable
   * Certificates)
   *
   ...
   * @throws KeyStoreException If there was a problem accessing the KeyStore.
   */
  public static boolean isSupportedEntryType(String alias, KeyStore keyStore)
      throws KeyStoreException {
    Certificate certificate = keyStore.getCertificate(alias);
