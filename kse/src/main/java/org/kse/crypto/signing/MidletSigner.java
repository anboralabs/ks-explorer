...
public class MidletSigner {
  ...
  private static final String MIDLET_CERTIFICATE_ATTR =
      "MIDlet-Certificate-{0}-{1}";

  // MIDlet Certificate JAD attribute minus certificate chain number
  private static final String SUB_MIDLET_CERTIFICATE_ATTR =
      "MIDlet-Certificate-{0}-";
  ...
  public static void sign(File jadFile, File jarFile, RSAPrivateKey privateKey,
    ...
    sign(jadFile, tmpFile, jarFile, privateKey, certificateChain,
         certificateNumber);

    CopyUtil.copyClose(new FileInputStream(tmpFile),
                       new FileOutputStream(jadFile));
   ...
   */
  public static void
  sign(File jadFile, File outputJadFile, File jarFile, RSAPrivateKey privateKey,
    ...
    // provided number)
    for (Enumeration<?> enumPropNames = jadProperties.propertyNames();
      ...
      if (propName.startsWith(MessageFormat.format(SUB_MIDLET_CERTIFICATE_ATTR,
                                                   certificateNumber))) {
        continue;
      }
    ...
    for (int i = 0; i < certificateChain.length; i++) {
      ...
      try {
        ...
        throw new CryptoException(
            res.getString("Base64CertificateFailed.exception.message"), ex);
      }

      String midletCertificateAttr = MessageFormat.format(
          MIDLET_CERTIFICATE_ATTR, certificateNumber, (i + 1));
    ...
    for (Enumeration<?> names = newJadProperties.propertyNames();
    ...
    try (FileWriter fw = new FileWriter(outputJadFile)) {
      for (Iterator<Entry<String, String>> itrSorted =
           sortedJadProperties.entrySet().iterator();
           itrSorted.hasNext();) {
        Entry<String, String> property = itrSorted.next();

        fw.write(MessageFormat.format(JAD_ATTR_TEMPLATE, property.getKey(),
                                      property.getValue()));
        fw.write(CRLF);
      }
    }
  ...
  private static byte[] signJarDigest(File jarFile, RSAPrivateKey privateKey)
    ...
    try (FileInputStream fis = new FileInputStream(jarFile)) {
      ...
      throw new CryptoException(
          res.getString("JarDigestSignatureFailed.exception.message"), ex);
    }
  }
   ...
   * @throws IOException If an I/O problem occurred or supplied file is not a
   *     JAD file
   */
      ...
      while ((line = lnr.readLine()) != null) {
        ...
        if (index == -1) {
          throw new IOException(
              res.getString("NoReadJadCorrupt.exception.message"));
        }
