...
public class Pkcs10Util {
  private static ResourceBundle res =
      ResourceBundle.getBundle("org/kse/crypto/csr/pkcs10/resources");
  private static final String BEGIN_CSR_FORM_1 =
      "-----BEGIN CERTIFICATE REQUEST-----";
  private static final String END_CSR_FORM_1 =
      "-----END CERTIFICATE REQUEST-----";
  private static final String BEGIN_CSR_FORM_2 =
      "-----BEGIN NEW CERTIFICATE REQUEST-----";
  private static final String END_CSR_FORM_2 =
   ...
   * @param extensions       Optional extensions from cert for extensionRequest
   *     attribute, pass null if not required
  ...
  generateCsr(X500Principal subjectDN, PublicKey publicKey,
              PrivateKey privateKey, SignatureType signatureType,
              String challenge, String unstructuredName, Extensions extensions,
    ...
    try {
      ...
      if (challenge != null) {
        ...
        csrBuilder.addAttribute(pkcs_9_at_challengePassword,
                                new DERUTF8String(challenge));
      }

      if (unstructuredName != null) {
        csrBuilder.addAttribute(pkcs_9_at_unstructuredName,
                                new DERUTF8String(unstructuredName));
      }

      if (extensions != null) {
        csrBuilder.addAttribute(pkcs_9_at_extensionRequest,
                                extensions.toASN1Primitive());
      }

      // fall back to bouncy castle provider if given provider does not support
      // the requested algorithm
      if (provider != null &&
          provider.getService("Signature", signatureType.jce()) == null) {
        provider = KSE.BC;
      }
      ...
      if (provider == null) {
        contentSigner =
            new JcaContentSignerBuilder(signatureType.jce()).build(privateKey);
      } else {
        contentSigner = new JcaContentSignerBuilder(signatureType.jce())
      ...
      if (!verifyCsr(csr)) {
        throw new CryptoException(
            res.getString("NoVerifyGenPkcs10Csr.exception.message"));
      }

      return csr;
    } catch (OperatorCreationException e) {
      throw new CryptoException(
          res.getString("NoGeneratePkcs10Csr.exception.message"), e);
    }
  }
  ...
  public static boolean verifyCsr(PKCS10CertificationRequest csr)
    ...
    try {
      ...
      ContentVerifierProvider contentVerifierProvider =
          new JcaContentVerifierProviderBuilder().setProvider(KSE.BC).build(
              pubKey);
      return csr.isSignatureValid(contentVerifierProvider);
    } catch (InvalidKeyException | OperatorCreationException |
             NoSuchAlgorithmException | PKCSException e) {
      throw new CryptoException(
          res.getString("NoVerifyPkcs10Csr.exception.message"), e);
    }
  }
   ...
   */
  public static byte[] getCsrEncodedDer(PKCS10CertificationRequest csr)
      throws CryptoException {
    try {
      return csr.getEncoded();
    } catch (IOException e) {
      throw new CryptoException(
          res.getString("NoEncodePkcs10Csr.exception.message"), e);
    }
  }
  ...
  public static String getCsrEncodedDerPem(PKCS10CertificationRequest csr)
    ...
    try {
      ...
      throw new CryptoException(
          res.getString("NoPemPkcs10Csr.exception.message"), ex);
    }
  }
   ...
   */
  public static PKCS10CertificationRequest loadCsr(byte[] data)
      throws IOException {
    ...
    try (ByteArrayInputStream bais = new ByteArrayInputStream(data);
      ...
      if ((line != null) && (line.trim().equals(BEGIN_CSR_FORM_1) ||
                             line.trim().equals(BEGIN_CSR_FORM_2))) {
   ...
   */
  public static X509ExtensionSet
  getExtensions(PKCS10CertificationRequest pkcs10Csr) {
    ...
    if ((attributes != null) && (attributes.length > 0)) {
      ...
      if (attributeValues.length > 0) {
        ASN1Sequence asn1Sequence =
            ASN1Sequence.getInstance(attributeValues[0]);
