...
/**
 * Provide something like BC's SECNamedCurves/NISTNamedCurves/X962NamedCurves
...
public enum EdDSACurves {
  ...
  public int bitLength() { return bitLength; }

  /**
   * returns an enumeration containing the jce strings for curves contained in
   * this structure.
   */
  ...
  public static EdDSACurves resolve(ASN1ObjectIdentifier oid) {
    return Arrays.stream(EdDSACurves.values())
        ...
        .findFirst()
        .orElseThrow(
            () -> new InvalidParameterException("Unknown OID: " + oid.getId()));
  }
}
