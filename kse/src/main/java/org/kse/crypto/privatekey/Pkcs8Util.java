...
public class Pkcs8Util {
  ...
  public static String getPem(PrivateKey privateKey) {
    PemInfo pemInfo =
        new PemInfo(PKCS8_UNENC_PVK_PEM_TYPE, null, privateKey.getEncoded());
   ...
   */
  public static byte[] getEncrypted(PrivateKey privateKey, Pkcs8PbeType pbeType,
                                    Password password)
      throws CryptoException, IOException {
    try {
      JcePKCSPBEOutputEncryptorBuilder encryptorBuilder =
          new JcePKCSPBEOutputEncryptorBuilder(pbeType.oid());
      ...
      PKCS8EncryptedPrivateKeyInfo encryptedPrivateKeyInfo =
          new PKCS8EncryptedPrivateKeyInfoBuilder(get(privateKey))
              .build(encryptor);

      return encryptedPrivateKeyInfo.getEncoded();
    } catch (OperatorCreationException ex) {
      throw new CryptoException("NoEncryptPkcs8PrivateKey.exception.message",
                                ex);
    }
  }
   ...
   * @throws IOException     If an I/O error occurred
   */
  public static String getEncryptedPem(PrivateKey privateKey,
                                       Pkcs8PbeType pbeType, Password password)
      throws CryptoException, IOException {
    PemInfo pemInfo = new PemInfo(PKCS8_ENC_PVK_PEM_TYPE, null,
                                  getEncrypted(privateKey, pbeType, password));
  ...
  public static PrivateKey load(byte[] pvkData)
    ...
    if (encType == null) {
      ...
      throw new CryptoException(
          res.getString("NotValidPkcs8.exception.message"));
    }

    if (encType == ENCRYPTED) {
      throw new PrivateKeyEncryptedException(
          res.getString("Pkcs8IsEncrypted.exception.message"));
    }
    ...
    try {
      ...
      throw new CryptoException(
          res.getString("NoLoadPkcs8PrivateKey.exception.message"), ex);
  ...
  public static PrivateKey loadEncrypted(byte[] pvkData, Password password)
    ...
    // If we haven't got the encrypted bytes via PEM then assume it is DER
    // encoded
    if (encPvk == null) {
      encPvk = pvkData;
    }
    ...
    try {
      ...
      PrivateKeyInfo privateKeyInfo =
          encryptedPrivateKeyInfo.decryptPrivateKeyInfo(decProv);
   ...
   */
  public static EncryptionType getEncryptionType(byte[] pkcs8)
      throws IOException {
  ...
  private static String getPrivateKeyAlgorithm(byte[] unencPkcs8)
    ...
    try (ASN1InputStream ais =
             new ASN1InputStream(new ByteArrayInputStream(unencPkcs8))) {
      ...
      try {
        ...
        throw new CryptoException(
            res.getString("NoUnencryptedPkcs8.exception.message"));
      }
