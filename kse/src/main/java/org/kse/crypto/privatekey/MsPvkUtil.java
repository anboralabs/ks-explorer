...
public class MsPvkUtil {
   ...
   */
  public static PrivateKey load(byte[] pvk)
    ...
    // Read and validate the reserved, magic number and key type fields - only
    // returns the latter
    ...
    if (encrypted != PVK_UNENCRYPTED) {
      throw new PrivateKeyEncryptedException(MessageFormat.format(
          res.getString("MsPvkIsEncrypted.exception.message"),
    ...
    if (saltLength !=
        UNENCRYPTED_SALT_LENGTH) // Specific length (0) for unencrypted PVK
    {
      throw new CryptoException(MessageFormat.format(
          res.getString("InvalidMsPvkSaltLengthField.exception.message"),
          Long.toHexString(saltLength),
    ...
    if (keyLength != (privateKeyBlob.length + BLOB_HEADER_LENGTH)) {
      throw new CryptoException(MessageFormat.format(
          res.getString("InvalidMsPvkKeyLengthField.exception.message"),
   ...
   * @throws IOException                    An I/O error occurred
   */
  public static PrivateKey loadEncrypted(byte[] pvk, Password password)
      throws IOException, CryptoException {
    try {
      // Wrap in a byte buffer set up to read little endian
      ByteBuffer bb = ByteBuffer.wrap(pvk);
      ...
      if (encrypted != PVK_ENCRYPTED) {
        throw new PrivateKeyUnencryptedException(MessageFormat.format(
            res.getString("MsPvkIsUnencrypted.exception.message"),
      ...
      if (saltLength !=
          ENCRYPTED_SALT_LENGTH) { // Specific length for encrypted PVK
        throw new CryptoException(MessageFormat.format(
            res.getString("InvalidMsPvkSaltLengthField.exception.message"),
            Long.toHexString(saltLength),
      ...
      // Validate key length - should be length of encrypted key blob plus blob
      // header
      if (keyLength != (encryptedPrivateKeyBlob.length + BLOB_HEADER_LENGTH)) {
        throw new CryptoException(MessageFormat.format(
            res.getString("InvalidMsPvkKeyLengthField.exception.message"),
            Long.toHexString(keyLength),
            Long.toHexString(encryptedPrivateKeyBlob.length +
                             BLOB_HEADER_LENGTH)));
      }

      // Decrypt key using RC4 with strong key
      byte[] decryptedPrivateKeyBlob =
          decryptPrivateKeyBlob(encryptedPrivateKeyBlob, strongKey);

      // Test for success
      if (decryptedPrivateKeyBlob == null) {
        // Failed - now try weak key
        decryptedPrivateKeyBlob =
            decryptPrivateKeyBlob(encryptedPrivateKeyBlob, weakKey);

        if (decryptedPrivateKeyBlob == null) {
          // Failed - could not decrypt - password is most likely incorrect
          throw new CryptoException(
              res.getString("NoDecryptMsPvkCheckPassword.exception.message"));
        }
      }

      return blobToPrivateKey(decryptedPrivateKeyBlob);
    } catch (GeneralSecurityException ex) {
      throw new CryptoException(res.getString("NoLoadMsPvk.exception.message"),
                                ex);
    }
  }
   ...
   * @return The encoding
   * @throws CryptoException Problem encountered while getting the encoded
   *     private key
   */
  public static byte[] get(RSAPrivateCrtKey privateKey, int keyType)
  ...
  private static byte[] getInternal(PrivateKey privateKey, int keyType)
      throws CryptoException {
    ...
    try {
      ...
      throw new CryptoException(res.getString("NoGetMsPvk.exception.message"),
                                ex);
    }
  }
   ...
   *     private key
   */
  public static byte[] getEncrypted(RSAPrivateCrtKey privateKey, int keyType,
                                    Password password, boolean strong)
      throws CryptoException {
   ...
   *     private key
   */
  public static byte[] getEncrypted(DSAPrivateKey privateKey, Password password,
                                    boolean strong) throws CryptoException {
    return getEncryptedInternal(privateKey, PVK_KEY_SIGNATURE, password,
                                strong);
  }

  private static byte[] getEncryptedInternal(PrivateKey privateKey, int keyType,
                                             Password password, boolean strong)
      throws CryptoException {
    // PVK encode a private key unencrypted
    ...
    try {
      ...
      if (strong) {
        ...
        encryptedPrivateKeyBlob =
            encryptPrivateKeyBlob(privateKeyBlob, strongKey);
      } else {
        // The weak version uses only 5 bytes of the key followed by 11 zero
        // bytes
        ...
        encryptedPrivateKeyBlob =
            encryptPrivateKeyBlob(privateKeyBlob, weakKey);
      }
  ...
  private static void writeReservedMagicKeyType(ByteBuffer bb, long keyType)
      throws IOException, CryptoException {
    ...
    if ((keyType != PVK_KEY_EXCHANGE) && (keyType != PVK_KEY_SIGNATURE)) {
      throw new CryptoException(MessageFormat.format(
          res.getString("InvalidMsPvkKeyTypeField.exception.message"),
          Long.toHexString(keyType), Long.toHexString(PVK_KEY_EXCHANGE),
  ...
  private static long readReservedMagicKeyType(ByteBuffer bb)
      throws IOException, CryptoException {
    /*
    ...
    if (magic != PVK_MAGIC_NUMBER) {
      throw new CryptoException(MessageFormat.format(
          res.getString("InvalidMsPvkMagicField.exception.message"),
    ...
    if (reserved != PVK_RESERVED) {
      throw new CryptoException(MessageFormat.format(
          res.getString("InvalidMsPvkReservedField.exception.message"),
  ...
  private static void writePrivateKeyBlobHeader(ByteBuffer bb, long keyType,
  ...
  private static void readPrivateKeyBlobHeader(ByteBuffer bb, long keyType)
      throws IOException, CryptoException {
    ...
    if (blobType != PRIVATE_KEY_BLOB) {
      throw new CryptoException(MessageFormat.format(
          res.getString("InvalidBlobHeaderTypeField.exception.message"),
          Integer.toHexString(blobType),
    ...
    if (blobVersion != CUR_BLOB_VERSION) {
      throw new CryptoException(MessageFormat.format(
          res.getString("InvalidBlobHeaderVersionField.exception.message"),
          Integer.toHexString(blobVersion),
    ...
    if (blobReserved != BLOB_RESERVED) {
      throw new CryptoException(MessageFormat.format(
          ...
          Integer.toHexString(blobReserved),
          Integer.toHexString(BLOB_RESERVED)));
    }
    ...
    if (keyType == PVK_KEY_SIGNATURE) {
      ...
      if ((keyAlgId != CALG_RSA_SIGN) && (keyAlgId != CALG_DSS_SIGN)) {
        throw new CryptoException(MessageFormat.format(
            res.getString("InvalidBlobHeaderKeyAlgIdField.exception.message"),
  ...
  private static PrivateKey blobToPrivateKey(byte[] privateKeyBlob)
      throws CryptoException {
  ...
  private static RSAPrivateCrtKey blobToRsaPrivateKey(byte[] rsaPrivateKeyBlob)
      throws CryptoException {
    try {
      ...
      if (magic != RSA_PRIV_MAGIC) {
        throw new CryptoException(MessageFormat.format(
            res.getString("InvalidRsaMagicField.exception.message"),
            Long.toHexString(magic), Long.toHexString(RSA_PRIV_MAGIC)));
      }

      long bitLength = UnsignedUtil.getInt(bb); // rsapubkey.bitlen

      // Byte lengths divisions may have remainders to take account for if not
      // factors of 16 and/or 8
      int add8 = 0;
      if ((bitLength % 8) != 0) {
        add8++;
      }
      ...
      BigInteger publicExponent = new BigInteger(
          Long.toString(UnsignedUtil.getInt(bb))); // rsapubkey.pubexp

      BigInteger modulus =
          readBigInteger(bb, (int)(bitLength / 8) + add8); // modulus
      BigInteger prime1 =
          readBigInteger(bb, (int)(bitLength / 16) + add16); // prime 1
      BigInteger prime2 =
          readBigInteger(bb, (int)(bitLength / 16) + add16); // prime 2
      BigInteger exponent1 =
          readBigInteger(bb, (int)(bitLength / 16) + add16); // exponent1
      BigInteger exponent2 =
          readBigInteger(bb, (int)(bitLength / 16) + add16); // exponent2
      BigInteger coefficient =
          readBigInteger(bb, (int)(bitLength / 16) + add16); // coefficient
      BigInteger privateExponent =
          readBigInteger(bb, (int)(bitLength / 8) + add8); // privateExponent
      ...
      throw new CryptoException(
          res.getString("NoConvertBlobToRsaKey.exception.message"), ex);
    } catch (GeneralSecurityException ex) {
      throw new CryptoException(
          res.getString("NoConvertBlobToRsaKey.exception.message"), ex);
    }
  ...
  private static DSAPrivateKey blobToDsaPrivateKey(byte[] dsaPrivateKeyBlob)
      throws CryptoException {
    try {
      ...
      if (magic != DSS_PRIV_MAGIC) {
        throw new CryptoException(MessageFormat.format(
            res.getString("InvalidDsaMagicField.exception.message"),
      ...
      throw new CryptoException(
          res.getString("NoConvertBlobToDsaKey.exception.message"), ex);
    } catch (GeneralSecurityException ex) {
      throw new CryptoException(
          res.getString("NoConvertBlobToDsaKey.exception.message"), ex);
    }
  ...
  private static byte[] rsaPrivateKeyToBlob(RSAPrivateCrtKey rsaPrivCrtKey)
    ...
    try {
      ByteBuffer bb = ByteBuffer.wrap(
          new byte[4096]); // 2316 sufficient for a 4096 bit RSA key
      bb.order(ByteOrder.LITTLE_ENDIAN);
      ...
      UnsignedUtil.putInt(bb,
                          (int)publicExponent.longValue()); // rsapubkey.pubexp
      ...
      writeBigInteger(bb, rsaPrivCrtKey.getPrimeP(),
                      (bitLength / 16) + add16); // prime1
      writeBigInteger(bb, rsaPrivCrtKey.getPrimeQ(),
                      (bitLength / 16) + add16); // prime2
      writeBigInteger(bb, rsaPrivCrtKey.getPrimeExponentP(),
                      (bitLength / 16) + add16); // exponent1
      writeBigInteger(bb, rsaPrivCrtKey.getPrimeExponentQ(),
                      (bitLength / 16) + add16); // exponent2
      writeBigInteger(bb, rsaPrivCrtKey.getCrtCoefficient(),
                      (bitLength / 16) + add16); // coefficient
      writeBigInteger(bb, rsaPrivCrtKey.getPrivateExponent(),
                      (bitLength / 8) + add8); // privateExponent

      return getBufferBytes(bb);
    } catch (IOException ex) {
      throw new CryptoException(
          res.getString("NoConvertKeyToBlob.exception.message"), ex);
    }
  }
  ...
  private static byte[] dsaPrivateKeyToBlob(DSAPrivateKey dsaPrivKey)
    ...
    try {
      ...
      ByteBuffer bb = ByteBuffer.wrap(
          new byte[512]); // 328 sufficient for a 1024 bit DSA key
      bb.order(ByteOrder.LITTLE_ENDIAN);
  ...
  private static byte[] decryptPrivateKeyBlob(byte[] encryptedPvk,
                                              byte[] rc4Key)
      throws CryptoException {
    ...
    try {
      ...
      throw new CryptoException(
          res.getString("PrivateKeyBlobRc4DecryptionFailed.exception.message"),
          ex);
    }
  }

  private static byte[] encryptPrivateKeyBlob(byte[] privateKeyBlob,
                                              byte[] rc4Key)
      throws CryptoException {
    ...
    try {
      ...
      throw new CryptoException(
          res.getString("PrivateKeyBlobRc4EncryptionFailed.exception.message"),
          ex);
    }
  }
  ...
  private static void writeBigInteger(ByteBuffer bb, BigInteger bigInteger,
