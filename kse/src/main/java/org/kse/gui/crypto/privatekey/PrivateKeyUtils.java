...
public class PrivateKeyUtils {
   ...
   * @throws IOException
   */
  /*public static void exportAsPkcs8(PrivateKey privateKey, String alias, JFrame
  frame, ApplicationSettings applicationSettings, ResourceBundle res) throws
  CryptoException, IOException { File exportFile = null;

      try {
          DExportPrivateKeyPkcs8 dExportPrivateKeyPkcs8 = new
  DExportPrivateKeyPkcs8(frame, alias,
          ...
          byte[] encoded = getPkcs8EncodedPrivateKey(privateKey, pemEncode,
  pbeAlgorithm, exportPassword);
  ...
  +
          ...
          String message =
  MessageFormat.format(res.getString("ExportKeyPairPrivateKeyAction.NoWriteFile.message"),
                                                exportFile);
  ...
  private static byte[] getPkcs8EncodedPrivateKey(PrivateKey privateKey,
    ...
    if (pemEncode) {
      if ((pbeAlgorithm != null) && (password != null)) {
        encoded = Pkcs8Util.getEncryptedPem(privateKey, pbeAlgorithm, password)
                      .getBytes();
      } else {
  ...
  private static void exportEncodedPrivateKey(byte[] encoded, File exportFile)
      throws IOException {
...
* @throws IOException
*/
  /*public static void exportAsPvk(PrivateKey privateKey, String alias, JFrame
  frame, ApplicationSettings applicationSettings, ResourceBundle res) throws
  CryptoException, IOException { File exportFile = null;

      try {
          DExportPrivateKeyPvk dExportPrivateKeyPvk = new
  DExportPrivateKeyPvk(frame, alias, privateKey,
                                                                               applicationSettings.getPasswordQualityConfig());
          ...
          byte[] encoded = getPvkEncodedPrivateKey(privateKey, keyType,
  exportPassword, strongEncryption);
  ...
  MessageFormat.format(res.getString("ExportKeyPairPrivateKeyAction.NoWriteFile.message"),
      ...
      private static byte[] getPvkEncodedPrivateKey(PrivateKey privateKey, int
  keyType, Password password, boolean strongEncryption) throws CryptoException,
  IOException { byte[] encoded = null;

              if (password != null) {
                      if (privateKey instanceof RSAPrivateCrtKey) {
                              encoded =
  MsPvkUtil.getEncrypted((RSAPrivateCrtKey) privateKey, keyType, password,
  strongEncryption); } else { encoded = MsPvkUtil.getEncrypted((DSAPrivateKey)
  privateKey, password, strongEncryption);
                      }
              } else {
                      if (privateKey instanceof RSAPrivateCrtKey) {
                              encoded = MsPvkUtil.get((RSAPrivateCrtKey)
  privateKey, keyType); } else { encoded = MsPvkUtil.get((DSAPrivateKey)
  privateKey);
                      }
              }
       ...
       * @throws IOException
       */
  /*public static void exportAsOpenSsl(PrivateKey privateKey, String alias,
  JFrame frame, ApplicationSettings applicationSettings, ResourceBundle res)
  throws CryptoException, IOException { File exportFile = null;

      try {
          DExportPrivateKeyOpenSsl dExportPrivateKeyOpenSsl = new
  DExportPrivateKeyOpenSsl(frame, alias,
                                                                                           applicationSettings.getPasswordQualityConfig());
          ...
          byte[] encoded = getOpenSslEncodedPrivateKey(privateKey, pemEncode,
  pbeAlgorithm, exportPassword);
  ...
  public static byte[] getOpenSslEncodedPrivateKey(PrivateKey privateKey,
    ...
    if (pemEncoded) {
      if ((pbeAlgorithm != null) && (password != null)) {
        encoded =
            OpenSslPvkUtil.getEncrypted(privateKey, pbeAlgorithm, password)
                .getBytes();
      } else {
