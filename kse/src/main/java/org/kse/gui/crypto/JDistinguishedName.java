...
public class JDistinguishedName extends JPanel {
  ...
  private static ResourceBundle res =
      ResourceBundle.getBundle("org/kse/gui/crypto/resources");
  ...
  private void initComponents(int columns) {
    ...
    ImageIcon viewEditIcon =
        new ImageIcon(getClass().getResource("images/view_edit_dn.png"));
    jbViewEditDistinguishedName = new JButton(viewEditIcon);

    if (editable) {
      jbViewEditDistinguishedName.setToolTipText(res.getString(
          "JDistinguishedName.jbViewEditDistinguishedName.Edit.tooltip"));
      ...
      jbViewEditDistinguishedName.setToolTipText(res.getString(
          "JDistinguishedName.jbViewEditDistinguishedName.View.tooltip"));
      jbViewEditDistinguishedName.addActionListener(evt -> {
        try {
          CursorUtil.setCursorBusy(JDistinguishedName.this);
    ...
    GridBagConstraints gbcJbViewEditDistinguishedName =
    ...
    ImageIcon clearIcon =
        new ImageIcon(getClass().getResource("images/clear_dn.png"));
    jbClearDistinguishedName = new JButton(clearIcon);
    jbClearDistinguishedName.setToolTipText(
        res.getString("JDistinguishedName.jbClearDistinguishedName.tooltip"));
  ...
  public void setDistinguishedName(X500Name distinguishedName) {
    ...
    if (distinguishedName == null) {
      this.distinguishedName =
          new X500Name(KseX500NameStyle.INSTANCE, new RDN[0]);
    } else {
      this.distinguishedName =
          new X500Name(KseX500NameStyle.INSTANCE, distinguishedName.getRDNs());
    }
    populate();
  }
   ...
   */
  public X500Name getDistinguishedName() { return distinguishedName; }
  ...
  private void displayDistinguishedName() {
    ...
    if (container instanceof JDialog) {
        DDistinguishedNameChooser dDistinguishedNameChooser = new
    DDistinguishedNameChooser((JDialog) container, title, distinguishedName,
                                                                                            false);
        dDistinguishedNameChooser.setLocationRelativeTo(container);
        dDistinguishedNameChooser.setVisible(true);
    } else if (container instanceof JFrame) {
        DDistinguishedNameChooser dDistinguishedNameChooser = new
    DDistinguishedNameChooser((JFrame) container, title, distinguishedName,
                                                                                            false);
        dDistinguishedNameChooser.setLocationRelativeTo(container);
        dDistinguishedNameChooser.setVisible(true);
    }*/
  ...
  private void editDistinguishedName() {
    ...
    if (container instanceof JDialog) {
        dDistinguishedNameChooser = new DDistinguishedNameChooser((JDialog)
    container, title, distinguishedName, true); } else {
        dDistinguishedNameChooser = new DDistinguishedNameChooser((JFrame)
    container, title, distinguishedName, true);
    }
    dDistinguishedNameChooser.setLocationRelativeTo(container);
    dDistinguishedNameChooser.setVisible(true);

    X500Name newDistinguishedName =
    dDistinguishedNameChooser.getDistinguishedName();
  ...
  private void clearDistinguishedName() { setDistinguishedName(null); }
}
