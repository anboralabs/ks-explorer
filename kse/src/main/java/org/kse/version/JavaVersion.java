...
public class JavaVersion implements Comparable<Object> {
  ...
  private static final String VERSION_FORMAT =
      "^" + VERSION_NUMBER_REGEXP + REST_REGEXP + "$";
  private static final Pattern VERSION_STRING_PATTERN =
      Pattern.compile(VERSION_FORMAT);
   ...
   */
  public JavaVersion() { this(System.getProperty("java.version")); }
  ...
  public JavaVersion(String javaVersion) {
    ...
    if (!matcher.matches()) {
      throw new VersionException(MessageFormat.format(
          res.getString("NoParseJavaVersion.exception.message"), javaVersion));
    }
